...
import './tool.scss'; // Если у тебя есть стили для этой категории
...
const Tool = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:3333/products?category=5'); // Запрос на сервер для получения товаров категории "Listic"
                if (!response.ok) {
    ...
    return (
        <div className="tool">
            <h2>Товары из категории "Tool"</h2>
