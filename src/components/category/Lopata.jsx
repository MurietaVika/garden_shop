...
import './lopata.scss'; // Если у тебя есть стили для этой категории
...
const Lopata = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:3333/products?category=3'); // Запрос на сервер для получения товаров категории "Listic"
                if (!response.ok) {
    ...
    return (
        <div className="lopata">
            <h2>Товары из категории "Lopata"</h2>
