...
import './listic.scss'; // Если у тебя есть стили для этой категории
...
const Listic = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:3333/products?category=2'); // Запрос на сервер для получения товаров категории "Listic"
                if (!response.ok) {
    ...
    return (
        <div className="listic">
            <h2>Товары из категории "Listic"</h2>
